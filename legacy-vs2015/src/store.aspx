...
<body>
  ...
  async function loadProducts(){ try{ var list=await KA.api('/api/products');
    ...
    var h=''; list.forEach(function(p){
      var sb = p.status==='Approved'?'<span class="badge badge-green">Live</span>':(p.status==='Rejected'?'<span class="badge badge-red" title="'+(p.rejectionReason||'')+'">Rejected</span>':'<span class="badge badge-amber">Pending approval</span>');
      h+='<div class="card" style="margin-bottom:10px"><div class="card-b"><div class="row"><strong>'+p.name+'</strong> '+(p.brand?('· '+p.brand):'')+' <span class="right">'+sb+'</span></div>'+
